    ...
    <div class="bg-[#212328] w-[80px] flex-col flex h-screen">
      ...
      <div class="flex-grow flex items-center flex-col pt-12 gap-4">
        <NuxtLink to="/" class="p-1 bg-gray-50 rounded-md bg-opacity-10">
          <Icon name="material-symbols:list-rounded" class="cursor-pointer w-8 h-8  hover:text-primary transition-all"
      ...
      <div v-if="user?.user_metadata" class="mt-auto mb-8 mx-auto flex flex-col items-center gap-8">
        <img :src="user.user_metadata.avatar_url" :class="{ 'opacity-50': !isLoad }" @load="onLoad" @error="onError"
          class="p-[1.5px] bg-gray-400 rounded-full w-[44px] h-[44px]" />
        <section class="flex flex-col cursor-pointer group" @click="onLogout">
          <Icon name="ic:outline-exit-to-app"
            class=" w-8 h-8 text-gray-50 opacity-20 group-hover:opacity-100 transition-all" />
          <span class="text-[12px] text-center text-white">登出</span>
        </section>
      </div>
    </div>
