export const URL_GET_DEMO_DATA = 'https://jsonplaceholder.typicode.com/posts';
export const URL_BLOCK_GROUP = '/api/block_group';
export const URL_UPDATE_BLOCK_GROUP_BAN = '/api/ban_block_group';
export const URL_SUBSCRIBE_BLOCK_GROUP = '/api/subscribe';
export const URL_MY_SUBSCRIBE_BLOCK_GROUP = '/api/my_subscribe';
export const URL_UPDATE_BLOCK_GROUP_LIKE = '/api/block_group_like';
export const URL_SEARCH_BLOCK_RECORD = '/api/block_record/search';
